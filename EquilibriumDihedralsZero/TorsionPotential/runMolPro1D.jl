...
d1::Float64 = 60.000 # 61.43364279
d2::Float64 = 180.00000000
d3::Float64 = 300.00000000 # 298.56635721
SaEq::Float64 = 0.00000000 #-1.7558466544600673
SbEq::Float64 = 0.00000000 # 3.04121561582463
tau::Float64 = 60.00
...
function SubmitJob(point::Int64, grid::Vector{Float64})
    submission::Cmd = `qsub -e CH3OH_1D_MEP_VQZ_AdaptiveGrid_$(point).e -o CH3OH_1D_MEP_VQZ_AdaptiveGrid_$(point).o -l h_rt="11:59:00" -l mem=30G -l tmpfs=100G GenerateMolproScript1D.csh $(point) $(grid[1]) $(grid[2]) $(grid[3]) $(grid[4]) $(grid[5]) $(grid[6]) $(grid[7]) $(grid[8]) $(grid[9]) $(grid[10]) $(grid[11]) $(grid[12])`
    run(submission)
end
...
#         displacementVector::Vector{Float64} = zeros(12)
#         displacementVector[i] = stretchesSpacing[j]
#         grid::Vector{Float64} = equilibriumGrid .+ 0
#         equilibriumBondLengths = [EqCH(tau), EqCH(tau + 120), EqCH(tau + 240)]
#         grid[3:5] = ConvertFromSymmeterisedStretches(displacementVector[3:5], equilibriumBondLengths)
#         if point >= startPoint
#             SubmitJob(point, grid)
#         end
#         PrintGeometry(grid)
...
#         displacementVector[i] = angleSpacing[j]
#         grid::Vector{Float64} = equilibriumGrid .+ 0
#         equilibriumAngles = [EqaHCO(tau), EqaHCO(tau + 120), EqaHCO(tau + 240)]
#         grid[7:9] = ConvertFromSymmeterisedAngles(displacementVector[7:9], equilibriumAngles)
#         if point >= startPoint
#             SubmitJob(point, grid)
#         end
#         PrintGeometry(grid)
