...
function SubmitJob(point::Int64, grid::Vector{Float64})
    submission::Cmd = `qsub -e CH3OH_1D_MEP_VQZ_AdaptiveGrid_$(point).e -o CH3OH_1D_MEP_VQZ_AdaptiveGrid_$(point).o -l h_rt="11:59:00" -l mem=20G -l tmpfs=100G GenerateMolproScript1D.csh $(point) $(grid[1]) $(grid[2]) $(grid[3]) $(grid[4]) $(grid[5]) $(grid[6]) $(grid[7]) $(grid[8]) $(grid[9]) $(grid[10]) $(grid[11]) $(grid[12])`
    run(submission)
end
